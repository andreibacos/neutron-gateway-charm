...
from neutron_contexts import (
    CORE_PLUGIN, OVS, LB, NSX, N1KV, OVS_ODL,
...
NEUTRON_LB_CONFIG_FILES.update(NEUTRON_SHARED_CONFIG_FILES)
...
def resolve_config_files(plugin, release):
    ...
    if plugin == LB:
        # NOTE: deal with switch to ML2 plugin for >= icehouse
        drop_config = [NEUTRON_LB_AGENT_CONF]
        if release >= 'mitaka':
            # ml2 -> lb_agent
            drop_config = [NEUTRON_ML2_PLUGIN_CONF]
...
def get_topics():
    ...
    if 'neutron-plugin-linuxbridge-agent' in services():
        topics.append('linuxbridge_agent')
