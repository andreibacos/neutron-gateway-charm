...
class NeutronGatewayContext(NeutronAPIContext):
    ...
    def __call__(self):
        ...
        security_group = config('security-group')
        if security_group:
            ctxt['firewall_driver'] = 'neutron.agent.linux.iptables_firewall.IptablesFirewallDriver'
        else:
            ctxt['firewall_driver'] = 'neutron.agent.firewall.NoopFirewallDriver'
